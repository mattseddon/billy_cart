...
def test_datetime_helpers():
    ...
    is_future = DateTime.is_more_than_33_seconds_from_now("2019-01-01T00:00:00.000Z")
    ...
    is_not_future = DateTime.is_more_than_33_seconds_from_now("1984-11-07T12:45:34.000Z")
    THEN("the date time is far enough in the future to return true")
    assert is_not_future == True

    WHEN("we call is_datetime_more_than_33_seconds_from_now with a time 33 seconds in the future")
    is_not_future = DateTime.is_more_than_33_seconds_from_now("1984-11-07T12:44:33.000Z")
    ...
    is_not_future = DateTime.is_more_than_33_seconds_from_now("1984-11-07T12:45:00.000Z")
    THEN("the date time is not far enough in the future to return true")
    assert is_not_future == False

    WHEN("we call is_datetime_more_than_33_seconds_from_now with a time from the past")
    is_not_future = DateTime.is_more_than_33_seconds_from_now("1984-11-07T12:44:00.000Z")
    THEN("the date time is not far enough in the future to return true")
    assert is_not_future == False
...
def test_date_time_str_converters():
    ...
    hour_minutes_seconds_str =  DateTime.local_hms_45_seconds_before(utc_date_time_str)
    ...
    hour_minutes_seconds_str =  DateTime.local_hms_30_seconds_before(utc_date_time_str)
    THEN("a string in the correct format corresponding to the local date time is returned")
