...
def test_create_df():
    ...
    assert len(data.odf.columns) == 15
    THEN("the dataframe has more records than the object's raw data")
    assert len(data.odf.index) > len(data._raw_data)
    assert type(data.odfT) is DataFrame
    assert len(data.odfT.index) < len(data.odf.index)
    assert len(data.odfT.index) == len(data._raw_data)
