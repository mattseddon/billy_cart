...
def test_real_comp_data():
    ...
    expected__data = __calc_compositional_data(
        items=items, correct_probability=correct_probability
    )

    WHEN("we calculate the compositional data")
    compositional_data = handler._get_compositional_data(
        price_name="price", correct_probability=correct_probability
    )

    THEN("a list of dictionaries with the correct values is returned")
    for idx, item in enumerate(compositional_data):
        for key in item.keys():
            assert almost_equal(item.get(key), expected__data[idx].get(key))
    assert almost_equal(
...
def __calc_compositional_data(items, correct_probability):
    expected__data = make_copy(items)
    pricer = PriceHandler()
    total_probability = 0

    for item in expected__data:
