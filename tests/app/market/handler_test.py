...
def test_system_set_prob(mock_post_instructions, mock_set_prob, mock_call_exchange):
    ...
    handler = MarketHandler(market_id=market_id, external_api=external_api)
    external_api.set_mediator(mediator=handler)
    mock_post_instructions.side_effect = __mark_orders_successful
    for raw_record in raw_data:
        mock_call_exchange.return_value = [raw_record]
        handler.run()
...
def test_exit_run_on_closed(mock_open_url):
    ...
    handler = MarketHandler(market_id=123456, external_api=external_api)
    external_api.set_mediator(mediator=handler)
    mock_open_url.return_value = __get_closed_market_dict()
...
def test_exit_run_on_no_data(mock_call_exchange):
    ...
    handler = MarketHandler(market_id=45678, external_api=external_api)
    external_api.set_mediator(mediator=handler)
