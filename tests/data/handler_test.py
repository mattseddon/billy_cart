...
def test_create_df():
    ...
    WHEN("we feed the data into the handler one record at a time")
    for i, raw_record in enumerate(raw_data):
        data.add(raw_record)
        number_records_processed = i + 1
        THEN("the dataframe the correct number of records")
        assert len(data.odf.index) == number_records_processed
 
    WHEN("we have finished")
    THEN("the dataframe has the correct number of columns")
    assert len(data.odf.columns) == number_runners* __get_number_columns()
    THEN("the dataframe has the same number of records as the raw data")
    assert len(data.odf.index) == len(raw_data)
    THEN("a sub frame has the correct number of runners in it")
    assert len(data.odf['sp_back'].columns) == number_runners
