...
def test_missing_market_info():
    GIVEN("a empty input dictionary and a record adapter")
    input = {
        "marketStartTime": "2019-01-13T05:25:00.000Z",
...
def test_valid_record():
    ...
    number_items = len(input.get("marketInfo")[0].get("runners"))
...
def test_mostly_valid_record():
    ...
    input = __get_data()
    for i in range(2):
        del input["marketInfo"][0]["runners"][i]["selectionId"]
    adapter = RecordAdapter()

    WHEN("we convert the input")
    adapted_data = adapter.convert(input)
    number_items = len(input.get("marketInfo")[0].get("runners")) - 2
