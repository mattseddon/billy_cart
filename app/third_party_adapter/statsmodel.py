...
from numpy import log, nan, isnan, float64
...
class WeightedLinearRegression:
    def __init__(self, y, x, weights):
        ...
        if self.__inputs_lengths_are_valid():
            self.__run_weighted_linear_regression()
        self.__data = {"alpha": self.__alpha, "Beta": self.__Beta}

    def get(self, key):
        return self.__data.get(key)

    def __run_weighted_linear_regression(self):
    ...
    def __calc_log_weights(self, weights):
        return [log(max(weight, 1)) for weight in weights]
    ...
    def __calc_valid_records(self):
        ...
        for i, item in enumerate(self.__weights):
            if (
                not isnan(item)
                and item > 0
                and not isnan(self.__x[i])
                and not isnan(self.__y[i])
            ):
                valid_records += 1
